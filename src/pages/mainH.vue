...
<script>
import list from './main/menuList.vue';
export default {
    name: 'VueMatrixRaindrop',
    components: {
        list,
    },
    //插件的各种参数
    ...
    methods: {
        ...
        initCanvas() {
            ...
            if (this.canvas.tagName.toLowerCase() !== 'canvas') {
                console.error(
                    "Error! Invalid canvas! Please check the canvas's id!",
                );
            }
        ...
        animationUpdate() {
            ...
            if (this.speedCnt === this.speed) {
                ...
                for (
                    var i = 0, len = this.rainDropPositionArray.length;
                    i < len;
                    i++
                ) {
                    ...
                    var textYPostion =
                        this.rainDropPositionArray[i] * this.fontSize;
