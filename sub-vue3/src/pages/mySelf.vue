<!-- 个人简介 -->
<template>
    <div class="mySelf">
        <!-- 两套 个人 和简历 -->
        <div v-if="compnent" id="pdfDom" class="row" style="padding-top: 55px">
            <div class="resume">
                <h2 style="color: #fff">个人简历</h2>
                <div class="contact">
                    <div class="ximg">
                        <img src="../assets/myself.jpg" alt="" />
                    </div>
                </div>
                <div class="selfIntroduce">
                    <div class="name1">
                        <p>姓名:车明强</p>
                        <p>性别:男</p>
                        <p>年龄:24</p>
                        <p>电话:15650272396</p>
                        <p>email:cmq15650272396@163.com</p>
                        <p>地址:深圳</p>
                        <p>工作经验:3年</p>
                        <p>学历:本科</p>
                    </div>
                </div>
                <div class="skill">
                    <h3>专业技能</h3>
                    <p>
                        1、熟练使用html、css制作符合W3C规范页面（例如规范：盒子命名问题以及嵌套，css书写顺序）会使用h5、css3新技术，会使用bootstrap做网页的响应式页面；
                    </p>
                    <p>
                        2、在工作中可以使用原生js开发，有良好编码习惯（例如规范：变量命名
                        模块化开发）会使用es5 es6，或使用jQuery开发；
                    </p>
                    <p>3、熟悉使用Vue2全家桶 ，会使用Vue3配合TS开发项目；</p>
                    <p>
                        4、使用uniapp框架来完成多端开发，打包app(ios和安卓)，各种小程序和H5，解决uniapp本身bug和兼容问题，实现功能：接口数据渲染，分享，快捷登录等；
                    </p>
                    <p>5、了解前端SEO、web 性能优化、代码管理和优化；</p>
                    <p>6、了解git、svn版本控制工具，具有多人协作开发能力；</p>
                    <p>
                        7、了解webpack、gulp等前端打包工具进行工程化搭建以及开发；
                    </p>
                    <p>
                        8、了解使用node微服务技术、express前端开发框架，可以写接口，会使用mysql数据库；
                    </p>
                </div>
                <div class="experience">
                    <h4>工作经历</h4>
                    <div class="experience-1">
                        <p><i>公司:</i>城迈科技（南京）股份有限公司</p>
                        <p><i>时间:</i>2022.03-2023.04</p>
                    </div>
                    <p><i>职位:</i>前端工程师</p>
                    <h5 style="color: #fff">职责:</h5>
                    <p>1、负责官网功能迭代；</p>
                    <p>2、负责前端框架搭建，开发规范及文档建设;</p>
                    <p>3、项目重构；</p>
                    <div class="experience-1">
                        <p><i>公司:</i>深圳华启天成智能科技有限公司</p>
                        <p><i>时间:</i>2020.08-2021.12</p>
                    </div>
                    <p><i>职位:</i>前端工程师</p>
                    <h5 style="color: #fff">职责:</h5>
                    <p>1、负责PC、移动前端解决方案与实现效果；</p>
                    <p>
                        2、与后台工程师协作，完成数据交互、信息展现等开发工作；
                    </p>
                    <p>3、负责前端框架搭建，开发规范及文档建设;</p>
                    <p>4、维护及优化网站前端页面；</p>
                    <p>5、node服务搭建以及接口开发；</p>
                </div>
                <div class="project">
                    <h5>项目经验</h5>
                    <div class="project1">
                        <h6 style="color: #fff">项目一:</h6>
                        <p><i>项目名称:</i>oppo开放平台（js jq vue2 vue3）</p>
                        <p>
                            <i>项目描述:</i>OPPO
                            开放平台，为开发者提供强大的应用分发能力，丰富的应用服务及推广变现支持，助力开发者实现业务增长和商业变现。
                        </p>
                        <p><i>负责内容:</i>需求迭代、项目重构</p>
                        <!-- <p><i>项目难点:</i>页面布局</p> -->
                        <p>项目难点: 项目迭代 适配 iframe嵌入 ssi</p>
                        <em style="color: #fff"
                            >1、项目迭代困难
                            项目处于新旧替代，涉及开发量较大，故旧项目页面采用iframe嵌入方式。（尽管iframe会影响前端性能优化）旧项目使用jq进行开发。迭代中处理并梳理逻辑使用vue进行重构。</em
                        >
                        <em style="color: #fff"
                            >2、适配
                            官网开发较早导致有些配置不太完善甚至说不太合理。这里使用了媒体查询以及弹性盒模型进行适配。
                        </em>
                        <em style="color: #fff"
                            >3、ssi 微前端架构方式
                            原先采用nginx指令插入方式接入公司其他业务。后面迭代中
                            采用了qiankun微前端框架进行架构。</em
                        >
                        <em style="color: #fff"
                            >4、webpack升级成vite
                            将项目webpack配置更改为vite配置</em
                        >
                        <em style="color: #fff"
                            >5、本地代理问题
                            为了处理开发环境与测试环境分别读取不同库导致数据问题，故在本地做了代理。使用测试cookie代理测试环境域名</em
                        >
                        <em style="color: #fff"
                            >6、代码规范问题 因为开发历史较长
                            有不少代码质量问题检测出来
                            结合eslint插件统一处理</em
                        >
                        <em style="color: #fff"
                            >7、代码质量问题
                            代码功能实现可读性很差，扫描出质量问题。解决函数复杂度以及定义规范问题</em
                        >
                    </div>
                    <div class="project1">
                        <h6 style="color: #fff">项目二:</h6>
                        <p>
                            <i>项目名称:</i>oppo上架平台、云测平台后台 （vue3）
                        </p>
                        <p>
                            <i>项目描述:</i>OPPO
                            上架平台负责开发者平台官网的产品服务能力的配置
                            云测平台用于oppo实验机调试、后台管理控制以及预览功能
                        </p>
                        <p><i>负责内容:</i>需求迭代、项目重构、搭建</p>
                        <p>项目难点: 搭建项目 定制功能</p>
                        <em style="color: #fff"
                            >1、基于公司封装的结合vue脚手架以及webpack打包工具、el
                            ui组件库一套框架进行项目搭建。</em
                        >
                        <em style="color: #fff"
                            >2、基于定制功能的基础上在公司ui组件上封装扩展功能
                            如时间选择器与表格内容联动、搜索下拉框定制
                        </em>
                        <em style="color: #fff"
                            >3、登陆问题 在request做了一层拦截
                            如果遇到与后端协定好的接口失效状态值，将会处理跳转登陆处理，或在全局路由拦截那里进行接口请求拦截，基于状态进行判断。当然后者较消耗性能，可在路由即将跳转时优化。</em
                        >
                        <em style="color: #fff"
                            >4、路由懒加载以及组件状态缓存的合理使用，防止进入首屏或频繁切换带来的性能损耗问题。</em
                        >
                        <em style="color: #fff"
                            >5、3轮走测中
                            产品经理出于更多考虑将一页功能叠加到另一个功能区，因为最初制定的代码开发规范代码有了更好的延展性、可迭代性。</em
                        >
                    </div>
                    <div class="project1">
                        <h6 style="color: #fff">项目三:</h6>
                        <p>
                            <i>项目名称:</i
                            >华启天成智能科技有限公司crm系统（vue2）
                        </p>
                        <p>
                            <i>项目描述:</i>OPPO
                            该项目是对商品数据进行可视化管理，node微服务搞后台接口，数据库表、字段设计。
                        </p>
                        <p>
                            <i>负责内容:</i
                            >该项目是对商品数据进行可视化管理，node微服务搞后台接口，数据库表、字段设计。
                        </p>
                        <p>项目难点:</p>
                        <em style="color: #fff"
                            >1、项目从0到1，商定团队开发风格以及统一eslint规则开发，减少不必要的低级bug；</em
                        >
                        <em style="color: #fff"
                            >2、echars表是使用websocked实时变化更新
                            可以多方实时改变；
                        </em>
                        <em style="color: #fff"
                            >3、基于角色权限进行侧边栏动态渲染，数据库表分表设计基于角色
                            权限 以及角色权限表三表查询；</em
                        >
                        <em style="color: #fff"
                            >4、数据库表、字段的保留与设计，由于没有数据库经验，公司专门从总部调后端java的来指导。</em
                        >
                        <em style="color: #fff"
                            >5、操作记录表通过node后台来记录当前用户所做的修改以及删除操作记录。不好设计应该后端处理好数据直接返回给前台，还是前台做处理；</em
                        >
                        <em style="color: #fff"
                            >6、node中间件使用，添加基础的jwt、cors中间件，以及使用express框架对数据操作；根据需求对数据库进行查询增加修改删除等操作；</em
                        >
                        <em style="color: #fff"
                            >7、头次接触到阿里云的服务器，进行了cdn节点加上以及oss资源、linux系统安装服务部署项目</em
                        >
                    </div>
                    <div class="project1">
                        <h6 style="color: #fff">项目四:</h6>
                        <p><i>项目名称:</i>华启天成官网（H5）</p>
                        <p>
                            <i>项目描述:</i>OPPO
                            该项目是根据公司新产品研发进度，推出不同阶段时期的页面首页及产品页替换。客户可进行表单提交产品订购意向及个人信息方便联系
                        </p>
                        <p>
                            <i>负责内容:</i
                            >项目技术选型、项目ui设计、后台搭建、webpack配置打包
                        </p>
                        <p>项目难点:</p>
                        <em style="color: #fff"
                            >1、没有设计ui，只能参考多个同行公司官网设计风格进行设计；
                        </em>
                        <em style="color: #fff"
                            >2、pc和移动两端适配，最后决定在加载是判断端，然后进行nginx反向代理。两套代码；
                        </em>
                        <em style="color: #fff"
                            >3、实现鼠标逐屏滚动，依据需求加载400张图片。依据需求做了以下改善：阿里云cdn节点分发；
                            照片压缩；判断鼠标滚动距离，懒加载图片，替换src标签；</em
                        >
                        <em style="color: #fff"
                            >4、使用i8n语言，用js判断当前浏览器使用的语言，然后实现英语、韩语、日文、中文之间切换；</em
                        >
                        <em style="color: #fff"
                            >5、主要采用了css3属性进行动态交互，减少js、jq调用dom操作。</em
                        >
                        <em style="color: #fff"
                            >6、压缩代码体积以及处理es6转es5 使用webpack配置</em
                        >
                    </div>
                    <div class="project1">
                        <h6 style="color: #fff">项目五:</h6>
                        <p><i>项目名称:</i>多宝配汇商城（wx）</p>
                        <p>
                            <i>项目描述:</i>OPPO 该项目是uniapp
                            二开项目，接手时残缺严重，基本界面样式以及功能都无，接手后独立开发，通过引入uniapp的view
                            ui库完善缺失部分的界面，渲染后端接口返回的富文本内容，首页为正常商城的轮播图以及商品展示部分，商品页为tab栏切换功能去渲染商品数据，购物车界面为商品选购结算等，我的界面为类似淘宝的待支付、待发货等等功能以及个人信息展示
                        </p>
                        <p>
                            <i>负责内容:</i
                            >持久化缓存、登录信息验证、封装模块化、打包
                        </p>
                        <p>项目难点:</p>
                        <em style="color: #fff"
                            >1、登录：手机短信登录、token、用户信息存储和判断；
                        </em>
                        <em style="color: #fff"
                            >2、搜索：对于商品（综合、价格、销量）排序和搜索，做持久化搜索处理；
                        </em>
                        <em style="color: #fff"
                            >3、我的：用户个人中心，地址管理，订单管理等并且做导航守卫拦截；</em
                        >
                        <em style="color: #fff"
                            >4、商品详情：展示商品详情、分享该商品到朋友圈、微信等；</em
                        >
                        <em style="color: #fff">5、项目模块优化和封装； </em>
                        <em style="color: #fff"
                            >6、项目打包：安卓 apk 包、ios 的 ipa 包；</em
                        >
                    </div>
                </div>

                <div class="education">
                    <h5>教育背景</h5>
                    <div class="education1">
                        <p>山东经贸职业学院</p>
                        <p>2017.09-2020.06</p>
                        <p>专业:计算机应用技术</p>
                    </div>
                    <div class="education1">
                        <p>长春工业大学</p>
                        <p>2022.03-2024.07</p>
                        <p>专业:工商管理</p>
                    </div>
                </div>

                <div class="selfEvaluation">
                    <h5>自我评价</h5>
                    <p>
                        1、做事谨慎细心，工作认真负责，具有高度的责任感，喜欢钻研开发中遇到的问题；
                    </p>
                    <p>
                        2、善于沟通，热爱技术工作，有较为清晰的目标和计划，注重团队协作精神；
                    </p>
                    <p>
                        3、热爱前端开发，对 Vue.js、react.js
                        前端组件化开发等技术有浓厚兴趣；
                    </p>
                </div>
            </div>
            <div class="leftBottomss" @click="myse">
                <img src="../assets/menu.png" alt="简历" title="个人介绍" />
            </div>
        </div>
        <div
            v-else
            id="pdfDom"
            class="row zz"
            style="padding-top: 55px; height: 100vh"
        >
            <div class="leftTop">2019-</div>
            <div class="leftBottom" @click="offer">
                <img src="../assets/menu.png" alt="个人简历" title="个人简历" />
            </div>
            <div class="rightTop">
                <p>DESIGN</p>
                <p>LEIYAJUN</p>
            </div>
            <div class="self">
                <div class="selfImg">
                    <img src="../assets/myself.jpg" alt="" />
                </div>
                <div class="selfTxt">
                    <i></i>
                    <h2>基本信息</h2>
                    <ul>
                        <li>
                            <svg
                                t="1637245360179"
                                class="icon"
                                viewBox="0 0 1024 1024"
                                version="1.1"
                                xmlns="http://www.w3.org/2000/svg"
                                p-id="20256"
                                width="32"
                                height="32"
                            >
                                <path
                                    d="M500 128.8c-95.2 5.6-173.6 83.2-180 178.4-7.2 112 80.8 205.6 191.2 205.6 106.4 0 192-86.4 192-192 0.8-110.4-92-198.4-203.2-192zM512 575.2c-128 0-383.2 64-383.2 192v96c0 17.6 14.4 32 32 32h702.4c17.6 0 32-14.4 32-32V766.4c0-127.2-255.2-191.2-383.2-191.2z"
                                    p-id="20257"
                                    data-spm-anchor-id="a313x.7781069.0.i36"
                                    fill="#b89a68"
                                    class="selected"
                                ></path></svg
                            >姓名:车明强
                        </li>
                        <li>
                            <svg
                                t="1637245413060"
                                class="icon"
                                viewBox="0 0 1186 1024"
                                version="1.1"
                                xmlns="http://www.w3.org/2000/svg"
                                p-id="22894"
                                width="32"
                                height="32"
                            >
                                <path
                                    d="M715.859172 61.320993q9.560943-21.650401 19.108717-43.300801c-12.524046-1.646168-42.629165-3.305505-55.15321-4.951673-7.01926-0.908685 14.301907 37.150718 21.070949 56.233097 2.067587 2.791901 6.584672 2.633869 9.337065 0.553112s4.240529-5.373092 5.636479-8.533735zM332.552261 168.848683l60.420948-28.024363c6.150083-2.857748 12.563554-5.781342 19.358935-5.807681 13.564424 0 24.837382 11.193942 38.322789 12.774264q-9.560943 29.314959-19.095548 58.643086a281.284007 281.284007 0 0 0-63.528913 39.810925 246.819835 246.819835 0 0 1 15.342285-52.572019 146.680147 146.680147 0 0 1-50.820496-24.824212z"
                                    fill="#ED758D"
                                    p-id="22895"
                                ></path>
                                <path
                                    d="M620.513125 13.64797c10.074548 3.950803 15.908567 14.565294 18.766314 25.021753 6.953413 25.311478 1.027209 52.75639-10.785692 76.197821s-29.143757 43.551019-46.290242 63.423558c-10.917386 12.655739-21.992804 25.443172-35.662582 35.069962-32.791665-35.478211-55.930202-68.822989-75.776402-115.495143-6.584672-15.368624-13.024481-37.269242-8.889307-53.454365s17.383533-31.066481 34.029583-32.383415c14.143875-1.119394 32.673141 11.931425 40.008466 24.07356 4.938504 8.191332 8.270348 14.341415 11.154433 23.546786a5.267737 5.267737 0 0 0 9.797992 0.553112c6.584672-15.803212 14.999882-28.972556 30.28949-39.205135 9.8375-6.571502 22.308868-11.602192 33.357947-7.348494z"
                                    fill="#EBDD9E"
                                    p-id="22896"
                                ></path>
                                <path
                                    d="M226.288829 603.555541l19.569644-249.624905a36.531759 36.531759 0 0 1 34.148108-33.621334 4225.462864 4225.462864 0 0 1 536.821947 2.251958c14.973544 1.014039 31.817134 3.068457 40.956658 14.973543 5.61014 7.308986 7.058768 16.883098 8.323025 26.009453 11.668038 84.44183 19.609152 165.538647 31.277191 249.980478z"
                                    fill="#F0D0D3"
                                    p-id="22897"
                                ></path>
                                <path
                                    d="M121.605717 1007.024716l12.313337-362.025251a40.087481 40.087481 0 0 1 40.522069-38.678362c261.911903 2.633869 519.030164-0.092185 786.209805 1.066717a40.271852 40.271852 0 0 1 39.942618 37.585306c8.099146 124.845376 16.211462 233.558306 24.323778 361.933066z"
                                    fill="#F0D0D3"
                                    p-id="22898"
                                ></path>
                                <path
                                    d="M241.354558 411.520175c2.225619 17.25184 9.495097 33.832043 18.147355 48.92411s25.245631 27.023493 42.51064 24.811044c18.121017-2.330974 31.053312-18.871669 38.52033-35.557228s11.760224-35.10947 22.848811-49.635255 31.89615-21.307998 49.54307-15.026221c27.300049 9.705806 22.717117 37.045363 29.920748 59.788819 6.769043 21.307998 27.813653 37.242903 50.14886 37.967217s44.380687-13.801472 52.519342-34.622204c9.337065-23.941866 5.544294-58.590409 29.578345-67.400699 16.145615-5.926205 33.818874 6.41347 42.234085 21.413352s11.009571 32.580956 18.344895 48.13395 22.229852 30.07878 39.284152 27.866331c19.819862-2.633869 29.854902-25.021753 34.240293-44.538719s8.388872-42.062883 25.627542-52.176939c19.46429-11.391482 45.118171 0.447758 59.512263 17.804952s21.953296 39.389506 35.267502 57.589539 36.50542 32.923359 57.708063 25.232462L857.798355 337.534803s-1.698845-16.027091-57.76074-16.027091-502.107558-2.172942-502.107558-2.172942-48.34466-4.385391-51.360439 30.539708-5.21506 61.645697-5.21506 61.645697zM128.96738 734.485153c6.492486 32.923359 13.696117 67.413869 35.451873 93.054581s63.094324 38.191096 90.249511 18.028832c37.782846-28.142887 31.909319-102.417984 78.383932-110.622486 33.107729-5.847188 56.351621 30.48703 68.664957 61.421818s32.09369 68.125014 65.662346 66.386661c34.740728-1.8042 50.267384-43.169108 59.051336-76.566563s30.434353-73.959033 64.79317-68.79665c30.368506 4.569762 41.654633 41.193706 47.238435 71.114454s19.174564 66.452507 49.832796 68.335724c32.278061 1.988571 50.241045-36.242033 56.522822-67.743103s16.05343-70.126754 47.52816-77.699127c27.365896-6.584672 53.16464 16.751405 65.293605 41.839005s16.777744 53.836276 32.923359 76.579732a61.369141 61.369141 0 0 0 7.993791 9.126355c23.704818 22.387884 63.041647 11.852409 75.170613-18.437081l33.318439-79.621851-12.563554-114.73132s-2.225619-18.871669-58.353361-18.871669H191.864165s-56.470145-5.070197-57.945111 37.70383z"
                                    fill="#AFCED3"
                                    p-id="22899"
                                ></path>
                                <path
                                    d="M314.813155 1023.815629c57.945111 0.092185 113.361708 0.184371 167.948637 0.184371 216.108926 0 418.337364-1.316934 703.848731-9.21854l-0.658467-23.704819c-52.677374 1.461797-102.352137 2.633869-149.919806 3.740094-4.21419-66.057427-8.42838-127.268535-12.629401-188.321612-3.661077-52.980269-7.322155-105.960537-10.970063-162.233142a52.203277 52.203277 0 0 0-51.716012-48.660724q-26.87863-0.171201-53.75726-0.171201c-4.872657-36.531759-9.21854-72.931824-13.459069-108.238834-4.833149-40.745949-9.82433-82.887848-15.671518-125.227287-1.316934-9.113186-3.00261-21.610893-10.667168-31.606424-13.169343-17.120146-36.874162-18.726806-49.55624-19.582814-84.494508-5.741834-169.568466-8.836629-254.695102-9.481927V215.454989c10.535475-9.047339 19.516967-19.372104 28.235072-29.459821 17.383533-20.149095 35.359687-40.969827 47.896902-65.846717a139.489686 139.489686 0 0 0 13.906827-43.458834q15.803212-7.993791 31.185005-16.711897c2.19928 4.912165 4.174682 9.534605 5.465278 13.169344l1.672506 3.147473a17.304517 17.304517 0 0 0 14.091198 6.861228 20.05691 20.05691 0 0 0 11.773393-3.832279c5.438939-3.950803 7.901606-9.613621 9.534605-13.314206l25.495848-57.787079-15.684688-2.054418c-6.41347-0.842838-17.317687-1.685676-28.208733-2.541683-10.43012-0.80333-20.847071-1.593491-26.957646-2.396821A12.207981 12.207981 0 0 0 679.538122 1.110755l-7.374832 0.447758-3.503045 5.465277c-4.345883 6.795381-1.975402 14.486278 5.583801 31.277191-6.940244 3.950803-13.854149 7.980622-20.939256 11.707546a92.356606 92.356606 0 0 0-2.633869-14.604802c-4.503915-16.461679-13.709287-28.142887-25.890929-32.923358-12.919126-5.043859-28.379935-2.080756-44.275332 8.546904a87.06253 87.06253 0 0 0-27.919009 30.579215c-1.870047-3.779602-4.01665-7.506526-6.584671-11.733885-9.086847-15.065729-31.869811-31.369376-51.083884-29.762716-20.478329 1.632999-38.796886 18.581944-44.578227 41.193706-5.386261 21.07095 3.6084 47.291112 9.455588 61.026738 4.082496 9.613621 8.362533 18.64779 12.84011 27.405404-5.373092 2.186111-10.746184 4.319545-16.158784 6.676857l-4.503916-0.526774c-4.253698-0.500435-8.955154-2.791901-13.959504-5.267737-7.335324-3.568892-15.64518-7.62505-25.785574-7.62505-9.508266 0-17.712767 3.845448-24.297439 6.900736l-78.304916 36.321049 15.711027 11.852409a160.165555 160.165555 0 0 0 42.352608 22.901488 259.541421 259.541421 0 0 0-11.391482 42.997907l-5.557463 31.777626 24.811043-20.623192a269.536953 269.536953 0 0 1 60.855536-38.12525l4.754133-2.159772 17.396703-53.454365q12.853279-5.794511 25.509018-10.864708c14.394092 24.231592 31.4879 46.566798 53.717752 70.614019l1.395951 1.514475v78.2259c-86.825481-0.171201-173.664132 2.238788-259.910163 7.374832a48.529031 48.529031 0 0 0-45.263033 44.762599l-18.924347 241.749637c-13.485408-0.105355-27.010323-0.158032-40.482561-0.302894-28.669661 0.171201-51.518472 21.742586-52.493003 50.13569l-3.134304 91.86934-1.632999 0.316064 1.382781 6.940244-8.731274 256.196408c-35.346518 0-71.838769 0-109.990357 0.065847v23.704818c116.996447-0.184371 219.519786-0.013169 314.813155 0.131693z m42.247254-853.268101l40.890812-18.924347c4.964842-2.304635 10.100886-4.688286 14.486277-4.714624 3.884956 0 8.164993 1.843708 12.971804 4.148343-16.409002 8.059638-33.371116 17.120146-51.202408 27.155186a135.80227 135.80227 0 0 1-17.146485-7.664558z m64.819509 27.300049a292.148715 292.148715 0 0 0-34.543188 19.411612c2.054418-6.439809 4.385391-12.800602 6.979752-19.029701l2.212449-5.267737q16.725066-9.21854 32.554617-17.185994zM706.482599 27.370426l11.022741 0.869177-10.719846 24.2711c-1.935893-4.398561-4.082496-9.03417-6.176422-13.590763-1.725184-3.713755-3.81911-8.244009-5.675987-12.471368z m-224.813862 65.846717c-7.835759-18.437081-10.943724-35.557227-8.309856-45.881992 2.80507-10.930555 11.760224-22.493239 24.297439-23.46777 8.573243 0 22.493239 9.126355 28.103379 18.43708 4.81998 7.980622 7.546034 13.169343 10.008701 20.978765a17.120146 17.120146 0 0 0 32.041012 1.553982c6.82172-16.356325 14.328246-26.180655 25.917268-33.924229 8.902476-5.965713 17.001622-8.296686 22.48007-6.176422 7.032429 2.739223 10.535475 12.853279 11.641699 17.120147a74.143404 74.143404 0 0 1 2.436329 19.754015A745.305824 745.305824 0 0 1 550.188831 94.955497l7.769913 22.387883c23.006843-7.980622 45.618606-17.120146 67.808949-27.313218a125.635537 125.635537 0 0 1-7.901606 19.503798c-11.220281 22.25619-28.221903 41.970698-44.670413 61.026737-7.993791 9.21854-16.198292 18.753145-25.324648 26.68109-29.828563-33.674011-49.358699-64.450767-66.202289-104.050982zM280.730894 332.148542a4231.178359 4231.178359 0 0 1 535.360151 2.238788c12.44503 0.842838 26.233332 2.40999 32.343907 10.364273 3.845448 5.01752 4.978012 13.169343 5.978882 20.425652 4.938504 35.70209 9.21854 71.114455 13.353714 105.960537-11.009571-2.949933-21.676739-11.681208-28.68283-21.25532-5.557463-7.62505-10.100886-16.079768-14.894527-25.021753-6.005221-11.193942-12.207981-22.769795-20.807563-33.147237-17.357195-20.939256-49.398207-35.201655-74.630669-20.45199-20.794393 12.181643-26.193824 36.874162-30.513369 56.628176l-0.711145 3.160643c-2.19928 9.969193-8.968323 33.371116-24.126237 35.346518-11.02274 1.514474-21.795263-9.929685-27.010323-21.07095a164.524608 164.524608 0 0 1-7.282647-20.122757c-3.042118-9.587282-6.071067-19.411612-11.338805-28.854031-10.719846-19.108717-34.240293-34.990946-56.628177-26.733767-22.756625 8.362533-27.115678 31.316699-30.947957 51.571149-1.580321 8.296686-3.068457 16.119276-5.623309 22.66444-6.347624 16.264139-23.889189 27.655621-41.075183 27.089339a43.827575 43.827575 0 0 1-39.244643-29.710039 130.126283 130.126283 0 0 1-3.674247-17.120146c-3.094796-18.200033-6.966583-40.824965-33.568656-50.333231-22.07182-7.835759-48.555369 0.144863-62.949462 19.016532-8.639089 11.325635-13.4459 24.126237-18.081509 36.50542-1.949063 5.267737-3.950803 10.43012-6.163252 15.500317-3.555723 7.901606-13.498577 26.615243-29.196435 28.630153-12.800602 1.593491-24.837382-8.678597-30.724078-18.950685-9.21854-16.158784-14.631141-30.421183-16.488018-43.603696l4.306375-56.009218a24.732027 24.732027 0 0 1 23.020012-22.717117z m-31.751287 133.747852a4.80681 4.80681 0 0 1 0.237049 0.447757c9.21854 16.027091 27.379065 31.040143 48.581708 31.040143a44.683582 44.683582 0 0 0 5.728664-0.381911c26.615243-3.41086 41.391246-28.07704 47.831055-42.471133 2.449498-5.491616 4.582932-11.167603 6.716366-16.84359 4.21419-11.272958 8.21767-21.900618 14.736495-30.460691 8.178162-10.706676 23.704818-15.460809 36.149848-11.03591 12.905957 4.596101 15.092068 14.117536 18.134186 31.975166a149.419371 149.419371 0 0 0 4.451238 20.228111c8.21767 25.904099 33.89789 45.355219 61.066245 46.237565s53.994308-16.84359 63.950332-42.141899c3.345013-8.586412 5.109705-17.883968 6.82172-26.865461 3.542553-18.674129 6.373962-30.28949 15.803212-33.753027s21.913788 5.596971 27.787315 16.079768c4.069327 7.256308 6.676857 15.552995 9.442419 24.336947a186.122331 186.122331 0 0 0 8.402041 23.046351c9.047339 19.398443 28.472121 37.637984 51.55798 34.622204 21.46603-2.80507 37.177057-21.874279 44.235825-53.74409l0.697975-3.22649c3.700586-16.883098 7.519695-34.332478 19.345765-41.272722 13.406392-7.901606 33.107729 1.514474 44.380688 15.131576 7.085107 8.546904 12.471368 18.581944 18.173694 29.209603 4.978012 9.21854 10.114056 18.871669 16.659219 27.826823 11.852409 16.2378 30.658232 30.131458 50.780989 31.961997 3.858618 32.515109 7.901606 65.846717 12.379182 99.48122-86.377724-0.158032-172.149658 0-255.722311 0.131694-127.202688 0.250218-258.119132 0.39508-388.495632-0.513605zM174.059212 618.173512h0.302895c151.684498 1.527644 304.857132 1.316934 453.025415 0.987701 108.51539-0.19754 220.744535-0.40825 333.18439 0.092185a28.406274 28.406274 0 0 1 28.156056 26.48355c2.080756 31.922489 4.148343 62.738752 6.229099 93.146766L962.679007 816.20093a36.610775 36.610775 0 0 1-26.206994 22.098158 32.515109 32.515109 0 0 1-29.828563-7.901606 50.188368 50.188368 0 0 1-6.452978-7.361663c-8.836629-12.44503-13.972673-27.28688-19.424782-42.984737a289.528016 289.528016 0 0 0-12.458199-31.856642c-13.999012-28.972556-44.617736-56.351621-78.713165-48.199797-41.035674 9.797992-51.031206 59.933682-56.404298 86.917667-5.531124 27.774145-20.307128 59.406908-44.169978 58.221667-24.573995-1.619829-34.91193-37.532629-38.862733-58.748441-3.437199-18.331726-13.880488-74.143404-57.141781-80.649059-40.91715-6.110575-67.163652 36.004985-78.02836 77.501586-11.852409 45.118171-27.168356 66.650047-48.199797 67.743103-20.004233 0.961362-37.835524-18.239541-53.994308-58.932812-19.635491-49.371869-49.411377-74.406791-81.728946-68.717634-32.580956 5.741834-44.301671 36.426404-54.626437 63.489404-7.322155 19.135056-14.868189 38.928579-28.788184 49.292853-21.808433 16.25097-56.56233 4.490746-74.130235-16.198293-18.147355-21.387014-25.390494-50.715142-30.934787-78.054698l3.186981-96.386425a28.050702 28.050702 0 0 1 28.287749-27.300049z m-18.674129 217.03078c24.87689 29.314959 73.577122 44.301671 106.355618 19.872539 19.45112-14.486278 28.261411-37.545798 36.768807-59.854666 10.746184-28.116548 18.555605-45.407896 36.610775-48.594877 28.630153-5.01752 48.423676 36.163017 55.574629 54.139171 7.361663 18.437081 29.420313 73.919525 74.41996 73.906356 0.948193 0 1.909555 0 2.870917-0.079017 43.1296-2.225619 60.57898-49.806457 69.902875-85.390022 7.901606-30.105119 26.193824-63.871316 51.584318-60.078545 17.883968 2.699715 30.105119 22.835642 37.348258 61.56668 9.047339 48.450015 30.631893 76.145144 60.750182 77.988852 39.033934 2.40999 61.118923-38.322789 68.875666-77.238199 6.439809-32.304399 15.579333-62.936292 38.652023-68.480586 20.412482-4.912165 41.549279 14.143875 51.860875 35.478211a269.800339 269.800339 0 0 1 11.444159 29.341297c5.755003 16.580203 11.694377 33.739858 22.493239 48.95045a73.300566 73.300566 0 0 0 9.534604 10.877878 55.943371 55.943371 0 0 0 51.189238 13.84098 60.315593 60.315593 0 0 0 43.037415-36.268372l13.946335-33.423794c0.368742 5.42577 0.750653 10.877878 1.119394 16.303647 4.187851 60.697504 8.375702 121.566209 12.563553 187.202218-263.386869 5.517955-459.74178 5.267737-697.448429 4.846318-57.194459-0.092185-117.128141-0.171201-181.13115-0.19754l6.400301-187.689483a123.291393 123.291393 0 0 0 15.276438 22.980504z"
                                    fill="#684C4B"
                                    p-id="22900"
                                ></path></svg
                            >年龄:1997/07/17
                        </li>
                        <li>
                            <svg
                                t="1637244955696"
                                class="icon"
                                viewBox="0 0 1024 1024"
                                version="1.1"
                                xmlns="http://www.w3.org/2000/svg"
                                p-id="8471"
                                width="32"
                                height="32"
                            >
                                <path
                                    d="M876.544 547.84l3.072 0-731.136 0 0-434.176q0-43.008 21.504-66.048t65.536-23.04l558.08 0q46.08 0 65.024 26.112t17.92 67.072l0 430.08zM800.768 96.256l-579.584 0 0 323.584 579.584 1.024 0-324.608zM563.2 484.352q0-14.336-19.456-16.384l-62.464 0q-19.456 0-19.456 16.384 0 5.12 5.12 10.24t13.312 6.144l61.44-1.024q9.216 0 15.36-5.12t6.144-10.24zM859.136 606.208q11.264 0 15.36 7.168t12.288 20.48q4.096 7.168 15.36 25.6t24.064 39.424 24.064 39.424 15.36 24.576q8.192 13.312 13.824 25.088t-7.68 28.16q-26.624 31.744-46.08 38.912t-29.696 7.168l-38.912 0-89.088 0-121.856 0-138.24 0-137.216 0q-67.584 0-121.344-0.512t-87.552-0.512l-38.912 0q-8.192 0-16.896-4.096t-17.92-10.752-16.896-14.336-13.824-14.848q-7.168-9.216-9.216-16.384t-0.512-12.8 5.632-10.752 8.192-12.288q4.096-6.144 13.824-22.016t20.992-35.328l24.576-38.912q11.264-19.456 19.456-32.768t13.824-19.968 11.776-8.704 13.312-1.536 18.432 0.512l661.504 0zM909.312 795.648q0-7.168-6.144-11.776t-13.312-4.608l-747.52 0q-7.168 0-14.336 4.608t-7.168 11.776 6.144 12.288 13.312 5.12l748.544 0q7.168 0 13.824-5.12t6.656-12.288z"
                                    p-id="8472"
                                    fill="#b89a68"
                                ></path></svg
                            >职业:前端开发工程师
                        </li>
                        <li>
                            <svg
                                t="1637245277997"
                                class="icon"
                                viewBox="0 0 1024 1024"
                                version="1.1"
                                xmlns="http://www.w3.org/2000/svg"
                                p-id="18058"
                                width="32"
                                height="32"
                            >
                                <path
                                    d="M742.218667 526.666667c107.637333 0 194.848 87.573333 194.848 195.552V725.333333c0 103.168-93.578667 190.4-190.4 190.4h-45.44a30.4 30.4 0 0 1-2.517334-0.106666c-153.898667-12.778667-312.266667-80.128-411.914666-173.888C169.184 631.082667 86.933333 461.44 86.933333 303.818667c0-113.802667 84.405333-195.552 194.848-195.552 107.626667 0 194.837333 87.573333 194.837334 195.552 0 76.032-44.181333 138.773333-111.701334 169.12a507.658667 507.658667 0 0 0 81.6 100.266666c36.288 34.453333 76.544 62.336 118.752 81.813334 26.304-76.170667 94.186667-128.352 176.96-128.352z"
                                    p-id="18059"
                                    data-spm-anchor-id="a313x.7781069.0.i31"
                                    fill="#b89a68"
                                    class="selected"
                                ></path></svg
                            >电话:15650272396
                        </li>
                        <li>
                            <svg
                                t="1637245232533"
                                class="icon"
                                viewBox="0 0 1024 1024"
                                version="1.1"
                                xmlns="http://www.w3.org/2000/svg"
                                p-id="15846"
                                width="32"
                                height="32"
                            >
                                <path
                                    d="M838.954667 234.666667H170.666667c-3.626667 0-7.168 0.448-10.56 1.322666l323.690666 323.669334a21.333333 21.333333 0 0 0 30.165334 0L838.954667 234.666667z m46.144 14.186666l-260.693334 260.693334 262.933334 262.912c5.44-7.168 8.661333-16.106667 8.661333-25.792V277.333333c0-10.944-4.117333-20.906667-10.88-28.48zM843.861333 789.333333l-249.6-249.621333-50.133333 50.133333a64 64 0 0 1-90.517333 0l-50.112-50.133333L156.373333 786.88c4.48 1.578667 9.28 2.453333 14.314667 2.453333h673.194667zM128.661333 754.218667L373.333333 509.525333 129.578667 265.813333A42.709333 42.709333 0 0 0 128 277.333333v469.333334c0 2.56 0.213333 5.098667 0.661333 7.552zM170.666667 192h682.666666a85.333333 85.333333 0 0 1 85.333334 85.333333v469.333334a85.333333 85.333333 0 0 1-85.333334 85.333333H170.666667a85.333333 85.333333 0 0 1-85.333334-85.333333V277.333333a85.333333 85.333333 0 0 1 85.333334-85.333333z"
                                    fill="#b89a68"
                                    p-id="15847"
                                ></path></svg
                            >邮箱:cmq15650272396@163.com
                        </li>
                    </ul>
                    <div>
                        <i></i>
                        <p>热爱生活 热爱代码</p>
                    </div>
                </div>
            </div>
        </div>
        <div v-if="!compnent" class="menu" @click="returnHome">
            <svg
                t="1637244786597"
                class="icon"
                viewBox="0 0 1024 1024"
                version="1.1"
                xmlns="http://www.w3.org/2000/svg"
                p-id="5956"
                width="32"
                height="32"
            >
                <path
                    d="M958.400956 451.54921c-0.058328-5.760191-2.597151-11.215436-6.965645-14.97097L524.345166 69.511143c-7.498788-6.445806-18.581194-6.445806-26.079982 0L309.582871 231.6755l0-102.017488c0-11.04966-8.901741-19.532869-19.951401-19.532869l-88.034009 0c-11.048637 0-19.928888 8.482185-19.928888 19.532869l0 211.954343L71.176063 436.57824c-4.423753 3.800559-6.967692 9.341762-6.967692 15.173584l0 105.500822c0 7.819083 4.554736 14.921851 11.660574 18.183128 2.670829 1.226944 5.51562 1.824555 8.343015 1.824555 4.699022 0 9.346879-1.654686 13.048177-4.836145l53.29788-45.825698 0 324.100516c0 60.677964 49.364291 110.042255 110.042255 110.042255L764.792447 960.741257c60.677964 0 110.042255-49.364291 110.042255-110.042255L874.834702 527.026228l51.585889 44.335764c5.955642 5.119601 14.356986 6.282077 21.481244 2.965541 7.122211-3.313465 11.645225-10.488889 11.565407-18.342764L958.400956 451.54921zM221.578538 150.034085l48.095391 0 0 115.941616-48.095391 41.336454L221.578538 150.034085zM570.718333 920.725892 436.666244 920.725892 436.666244 700.642404c0-11.031241 8.976442-20.007683 20.007683-20.007683l94.0357 0c11.031241 0 20.007683 8.976442 20.007683 20.007683L570.71731 920.725892zM834.818313 495.895207l0 354.803795c0 38.612413-31.414477 70.02689-70.02689 70.02689l-154.058748 0L610.732675 700.642404c0-33.096792-26.926256-60.023048-60.023048-60.023048l-94.0357 0c-33.096792 0-60.023048 26.926256-60.023048 60.023048l0 220.084511L260.59925 920.726915c-38.612413 0-70.02689-31.414477-70.02689-70.02689L190.57236 495.895207c0-1.172709-0.121773-2.314719-0.315178-3.432169l322.113255-276.958846 322.70268 277.348726C834.921667 493.848595 834.818313 494.858598 834.818313 495.895207zM525.411451 173.947727c-7.502881-6.445806-18.587334-6.446829-26.086122 0.00307L104.223736 513.663896l0-52.726875 407.081439-349.870436 407.176606 349.9523 0.521886 51.205219L525.411451 173.947727z"
                    p-id="5957"
                    fill="#b89a68"
                ></path>
            </svg>
        </div>
    </div>
</template>

<script lang="ts" setup>
import { ref } from 'vue';
import { useRouter } from 'vue-router';
const route = useRouter();
const compnent = ref(false);
const returnHome = () => {
    route.push('/');
};
const offer = () => {
    compnent.value = true;
};
const myse = () => {
    compnent.value = false;
};
</script>
<style lang="less" scoped>
.mySelf {
    width: 100vw;
    //height: 100vh;
    // background-color: aqua;
    background: linear-gradient(to bottom right, #000, #000);
}
// 简历
.resume {
    min-width: 1200px;
    max-width: 1600px;
    margin: 0 auto;
    h2 {
        text-align: center;
    }
    .contact {
        border-bottom: 1px solid #000;
        .ximg {
            width: 100px;
            height: 100px;
            margin: 0 auto;
            margin-top: 30px;
            border-radius: 50%; /*圆角*/
            -webkit-animation-play-state: paused;
            overflow: hidden;
            /* 动画名称 一次的时间 速度曲线 什么时候开始 循环次数 */
            img {
                width: 100px;
                height: 100px;
            }
        }
        .ximg:hover {
            -webkit-animation: run 5s linear 0.1s infinite;
        }
        /* 鼠标移动上去 暂停动画*/
        @keyframes run {
            from {
                -webkit-transform: rotate(360deg);
            }
            to {
                -webkit-transform: rotate(0deg);
            }
        }
        .shu {
            display: flex;
            justify-content: space-between;
            padding: 10px 20px;
        }
    }
    .selfIntroduce {
        padding-bottom: 40px;
        .name1 {
            display: flex;
            flex-wrap: wrap;
            justify-content: space-around;
            margin-top: 60px;
            width: 800px;
            p {
                width: 380px;
            }
        }
    }
    .skill {
        h3 {
            color: #fff;
            font-weight: 400;
            font-size: 16px;
            padding-left: 20px;
            line-height: 34px;
            background: linear-gradient(to right, #50a3a2, #53e3a6);
            margin-bottom: 20px;
        }
        p {
            padding-bottom: 10px;
            padding-left: 20px;
        }
    }
    .experience {
        margin-top: 30px;
        h4 {
            color: #fff;
            font-weight: 400;
            font-size: 16px;
            padding-left: 20px;
            line-height: 34px;
            background: linear-gradient(to right, #50a3a2, #53e3a6);
            margin-bottom: 20px;
        }
        .experience-1 {
            padding-top: 30px;
            display: flex;
            justify-content: space-between;
            padding-left: 20px;
            p {
                padding: 0 20px;
                i {
                    font-style: normal;
                    font-weight: bolder;
                }
            }
        }
        h5 {
            font-size: 16px;
            padding: 20px 0 20px 40px;
        }
        p {
            padding: 0 40px;
            i {
                font-style: normal;
                font-weight: bolder;
                padding-right: 10px;
            }
        }
    }
    .project {
        margin-top: 30px;

        h5 {
            color: #fff;
            font-weight: 400;
            font-size: 16px;
            padding-left: 20px;
            line-height: 34px;
            background: linear-gradient(to right, #50a3a2, #53e3a6);
            margin-bottom: 20px;
        }
        .project1 {
            margin-top: 20px;
            padding-bottom: 54px;
            border-bottom: 1px solid #000;
            h6 {
                font-size: 16px;
                margin-bottom: 20px;
            }
            p {
                padding-left: 30px;
                i {
                    font-style: normal;
                    font-weight: bolder;
                    margin-right: 10px;
                }
            }
            p:nth-child(7) {
                font-weight: bolder;
            }
            em {
                font-style: normal;
                display: block;
                padding-left: 96px;
            }
        }
    }
    .education {
        margin-top: 20px;
        h5 {
            color: #fff;
            font-weight: 400;
            font-size: 16px;
            padding-left: 20px;
            line-height: 34px;
            background: linear-gradient(to right, #50a3a2, #53e3a6);
            margin-bottom: 20px;
        }
        .education1 {
            display: flex;
            justify-content: space-around;
            padding: 20px 0;
        }
    }
    .selfEvaluation {
        padding-bottom: 40px;
        h5 {
            color: #fff;
            font-weight: 400;
            font-size: 16px;
            padding-left: 20px;
            line-height: 34px;
            background: linear-gradient(to right, #50a3a2, #53e3a6);
            margin-bottom: 20px;
        }
        p {
            padding-left: 20px;
        }
    }
}
// 个人
.self {
    width: 1200px;
    // height: 100%;
    margin: 0 auto;
    display: flex;
    margin-top: 200px;
    padding-bottom: 200px;
    // justify-content: space-between;
    .selfImg {
        width: 400px;
        height: 460px;
        // border: 6px dashed #fff;
        margin-left: 80px;
        img {
            width: 100%;
            height: 100%;
            display: block;
            border: 0;
        }
    }
    .selfTxt {
        // flex: 2;
        padding-left: 222px;
        padding-top: 20px;

        i {
            display: block;
            width: 24px;
            height: 4px;
            margin-bottom: 10px;
            background-color: #b89a68;
        }
        h2 {
            font-family: Cambria, Cochin, Georgia, Times, 'Times New Roman',
                serif;
            // margin-top: 10px;
            margin-bottom: 20px;
            color: #b89a68;
        }
        ul {
            li {
                margin-bottom: 10px;
                color: #b89a68;
                display: flex;
                font-size: 18px;
                align-items: center;
            }
        }
        div {
            margin-top: 40px;
            p {
                color: #b89a68;
            }
        }
    }
}
.zz {
    position: relative;
    .leftTop {
        position: absolute;
        left: 5%;
        top: 5%;
        color: #b89a68;
    }
    .leftBottom {
        position: absolute;
        left: 5%;
        bottom: 5%;
        cursor: pointer;
    }
    .rightTop {
        position: absolute;
        right: 5%;
        top: 5%;
        color: #b89a68;
        p {
            text-align: right;
        }
    }
}
.menu {
    cursor: pointer;
    position: fixed;
    bottom: 5%;
    right: 5%;
}
.leftBottomss {
    padding: 0 0 60px 60px;
    cursor: pointer;
}
p {
    color: #fff;
}
</style>
